...
from src.exceptions import (
    LLMError, LLMAPIKeyError, LLMTimeoutError, LLMNetworkError, LLMRateLimitError,
    LLMResponseParseError, ParsingError, UnsupportedFormatError, FileCorruptedError,
    VectorStoreError, VectorStoreInitError, RiskAnalysisError, RevisionError,
...
def test_llm_client_retry():
    ...
    with patch('requests.post') as mock_post:
        ...
        try:
            ...
            print("❌ 应该抛出 LLMTimeoutError")
        except LLMTimeoutError:
            print("✅ 正确抛出 LLMTimeoutError")
            print(f"✅ 重试次数: {mock_post.call_count} (期望: 3)")
...
def test_llm_client_api_key_error():
    ...
    with patch('requests.post') as mock_post:
        ...
        try:
            asyncio.run(client.chat_completion(prompt="测试"))
            print("❌ 应该抛出 LLMAPIKeyError")
        except LLMAPIKeyError:
            print("✅ 正确抛出 LLMAPIKeyError（不重试）")
            print(f"✅ 请求次数: {mock_post.call_count} (期望: 1)")
