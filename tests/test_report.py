...
class MockRiskItem:
    ...
    clause_position: Optional[str] = None
...
def single_risk_result():
    risk = MockRiskItem(
        id="R1", name="违约责任缺失", category="条款缺失",
        risk_level="high", description="未约定违约责任",
        clause_position="第五条", clause_content_preview="甲方应按时付款" * 5,
        legal_basis="民法典第五百七十七条", suggestion="补充违约责任条款",
        confidence=0.85, clause_title="第五条 付款方式",
        clause_line_range="第10-15行", cited_provisions=["民法典第五百七十七条"]
    )
...
def multi_level_risk_result():
    ...
    return MockRiskResult(
        risks=risks, critical_count=1, high_count=1, medium_count=1, low_count=1
    )
...
class TestGenerateReport:
    ...
    def test_report_with_security_warning(self, mock_freshness, generator, single_risk_result):
        ...
        report = generator.generate_report(
            "test.docx", "contract", single_risk_result, [],
            security_warning="检测到敏感信息"
        )
...
class TestClausePreview:
    ...
    def test_short_preview_no_ellipsis(self, mock_freshness, generator):
        ...
        risk = MockRiskItem(
            id="R1", name="风险A", risk_level="high",
            clause_content_preview="短文本"
        )
        result = MockRiskResult(risks=[risk], high_count=1)
        report = generator.generate_report("test.docx", "contract", result, [])
    ...
    def test_long_preview_has_ellipsis(self, mock_freshness, generator):
        ...
        risk = MockRiskItem(
            id="R1", name="风险A", risk_level="high",
            clause_content_preview=long_text
        )
        result = MockRiskResult(risks=[risk], high_count=1)
        report = generator.generate_report("test.docx", "contract", result, [])
        assert "…" in report
