...
class LoggerManager:
    ...
    def initialize(self, log_dir: str = "logs", enable_json_log: bool = True):
        ...
        # 文件日志（JSON 格式）
        if enable_json_log:
            self._add_file_handler("audit", self.log_dir / "audit.log", json_format=True)
            self._add_file_handler("llm_trace", self.log_dir / "llm_trace.log", json_format=True)
    ...
    def audit_file_upload(self, filename: str, file_size: int, file_type: str, user_id: str = "anonymous"):
        """记录文件上传审计"""
    ...
    def audit_review_start(self, filename: str, document_type: str, playbook: str, use_llm: bool):
        """记录审查开始"""
        self.audit_logger.info(
    ...
    def audit_review_complete(self, filename: str, risk_count: int, high_count: int, duration_seconds: float):
        """记录审查完成"""
        self.audit_logger.info(
    ...
    def log_llm_response(
        ...
        if error:
            self.llm_logger.error(
                ...
                extra=self._extra(
                    "llm_response_error", request_id=request_id, model=model, duration_ms=duration_ms, error=error
                ),
            )
        else:
            self.llm_logger.info(
