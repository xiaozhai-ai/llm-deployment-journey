...
class ToolRegistry:
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            return await asyncio.wait_for(tool.execute(tool_call.arguments, tool_call.id), timeout=self._timeout)
        except TimeoutError:
            return ToolResult(
                tool_call_id=tool_call.id,
                tool_name=tool_call.name,
            ...
            return ToolResult(
                tool_call_id=tool_call.id, tool_name=tool_call.name, success=False, content=f"工具执行失败: {str(e)}"
            )
