...
from src.core.data_models import BBox, LayoutBlock, PageLayout, TableCell, TableBlock
from src.parsing.layout.engine import LayoutEngine
...
class PaddleLayoutEngine(LayoutEngine):
    ...
    def analyze_image(self, image_bytes: bytes, page_index: int = 0) -> PageLayout:
        ...
        # 恢复正确的阅读顺序（多栏布局支持）
        from src.parsing.layout.reading_order import restore_reading_order
    ...
    def analyze_pdf(self, pdf_bytes: bytes) -> list[PageLayout]:
        ...
        # 跨页段落合并
        if len(results) > 1:
            from src.parsing.structure.cross_page_merger import merge_cross_page_paragraphs
    ...
    def _parse_table_result(
        ...
        # 尝试从 HTML 解析出结构化表格
        from src.parsing.structure.table_parser import parse_html_table
    ...
    def _table_html_to_text(html: str) -> str:
        ...
        from src.parsing.structure.table_parser import parse_html_table, table_to_plain_text
    ...
    def _detect_borderless_tables(
        ...
        from src.parsing.structure.table_parser import detect_borderless_table, table_to_plain_text
