...
from src.llm.tools.base import ToolCall, ToolRegistry
...
class StagnationDetector:
    ...
    def __init__(self, max_same_tool: int = 2, min_information_gain: float = 0.2, max_empty_results: int = 2):
        self.max_same_tool = max_same_tool
        self.min_information_gain = min_information_gain
        self.max_empty_results = max_empty_results

    def check(self, current_action: str, current_content: str, history: list[dict]) -> str | None:
        """
        检测是否应该中断
    ...
    def _detect_repeated_tools(self, current_action: str, history: list[dict]) -> str | None:
        """检测重复工具调用"""
        if current_action != "tool_call":
            return None

        # 获取最近的工具调用
        recent_calls = [h for h in history[-4:] if h.get("action") == "tool_call"]
        ...
        if last.get("tool_name") == second_last.get("tool_name") and self._normalize_args(
            last.get("tool_args")
        ) == self._normalize_args(second_last.get("tool_args")):
            return f"检测到重复调用同一工具「{last.get('tool_name')}」（相同参数），无新信息可获取，强制中断"

        # 检查最近 3 次是否都是同一工具（即使参数不同）
        if len(recent_calls) >= 3:
            tools = [c.get("tool_name") for c in recent_calls[-3:]]
            if len(set(tools)) == 1:
                return f"连续 3 次调用同一工具「{tools[0]}」，可能已穷尽该工具信息，强制中断"
    ...
    def _detect_empty_results(self, current_content: str, history: list[dict]) -> str | None:
        """检测空结果连续出现"""
        empty_indicators = ["未找到", "没有找到", "不存在", "无相关", "not found", "no results", "无匹配"]
        ...
        if consecutive_empty >= self.max_empty_results:
            return f"连续 {consecutive_empty} 次工具检索未找到结果，该领域可能缺乏相关法规/判例，强制中断"

        return None

    def _detect_low_information_gain(self, current_content: str, history: list[dict]) -> str | None:
        """检测信息增益不足"""
        # 获取上一轮工具结果
        prev_results = [h.get("content", "") for h in history[-4:] if h.get("action") == "tool_result"]
        ...
        if max_similarity > (1 - self.min_information_gain):
            return f"新检索结果与已有内容高度重复（相似度 {max_similarity:.0%}），信息增益不足，强制中断"
...
- 每个风险点附法律依据和修改建议"""
    ...
    async def analyze_stream(
        self, clause_text: str, context: str = "", role_context: str = ""
        ...
        messages = [
            {"role": "system", "content": system_prompt},
            {
                ...
                "content": (
                    ...
                    + "请分析该条款的合法性、潜在风险，并在需要时调用工具检索法规或判例来支撑你的判断。"
                ),
            },
        ]
        ...
        # 首轮思考
        yield AgentStreamEvent(type=EventType.THINKING, content="🔍 正在分析条款内容...", step_number=0)
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            yield AgentStreamEvent(
                type=EventType.THINKING, content=f"🤔 {step_prefix}：AI 正在分析...", step_number=iteration
            )
            ...
            if response.get("tool_calls"):
                ...
                messages.append(
                    {"role": "assistant", "content": response.get("content"), "tool_calls": assistant_tc_payload}
                )
                ...
                for idx, tc in enumerate(tool_calls):
                    ...
                    # 停滞检测 — 工具调用前
                    history.append(
                        {"action": "tool_call", "tool_name": tc["name"], "tool_args": tc.get("arguments", {})}
                    )

                    stop_reason = self.stagnation_detector.check("tool_call", "", history)
                    if stop_reason:
                        ...
                        yield AgentStreamEvent(
                            type=EventType.CONCLUSION, content=conclusion, step_number=iteration, is_final=True
                        )
                        return
                    ...
                    result = await self.tool_registry.execute(
                        ToolCall(id=tc_id, name=tc["name"], arguments=tc.get("arguments", {}))
                    )

                    # yield 工具结果
                    result_preview = result.content[:200] + ("..." if len(result.content) > 200 else "")
                    yield AgentStreamEvent(
                    ...
                    # 停滞检测 — 工具结果后
                    history.append({"action": "tool_result", "tool_name": tc["name"], "content": result.content})

                    stop_reason = self.stagnation_detector.check("tool_result", result.content, history)
                    if stop_reason:
                        self.logger.warning(f"Agent 强制中断: {stop_reason}")
                        yield AgentStreamEvent(
                            type=EventType.INTERRUPTED,
                        ...
                        verified = await self._self_reflect_and_finalize(draft, messages, history, iteration)
                        yield AgentStreamEvent(
                            type=EventType.REFLECTION, content=verified["reflection_summary"], step_number=iteration
                        )
                        yield AgentStreamEvent(
                    ...
                    # 将工具结果加入对话
                    messages.append({"role": "tool", "tool_call_id": tc_id, "content": result.content})
                ...
                # yield 反思事件
                yield AgentStreamEvent(
                    type=EventType.REFLECTION, content=verified_conclusion["reflection_summary"], step_number=iteration
                )
        ...
        draft = await self._generate_fallback_conclusion(messages)
        verified = await self._self_reflect_and_finalize(draft, messages, history, self.max_iterations)
        yield AgentStreamEvent(
            type=EventType.REFLECTION, content=verified["reflection_summary"], step_number=self.max_iterations
        )
        yield AgentStreamEvent(
            type=EventType.CONCLUSION,
            content=verified["final_conclusion"],
    ...
    async def analyze(self, clause_text: str, context: str = "", role_context: str = "") -> "AgentOutput":
        """
        分析条款（非流式，向后兼容）
        内部使用流式接口，收集所有事件后返回
        """
        ...
        async for event in self.analyze_stream(clause_text, context, role_context):
            if event.type == EventType.CONCLUSION and event.is_final:
                ...
                steps.append({"step_number": event.step_number, "action": "reflection", "content": event.content})
                ...
                steps.append({"step_number": event.step_number, "action": "thinking", "content": event.content})

        return AgentOutput(conclusion=conclusion, steps=steps, tools_used=tools_used)
    ...
    async def _generate_fallback_conclusion(self, messages: list[dict]) -> str:
        ...
        try:
            ...
            response = await self.llm_client.chat_completion(
                prompt="", messages=messages, temperature=0.1, max_tokens=2000
            )
            return response
    ...
    async def _self_reflect_and_finalize(
        self, draft_conclusion: str, messages: list[dict], history: list[dict], iteration: int
    ) -> dict:
        """
...
返回 JSON：{{"reflection_passed":bool,"corrections_made":int,"reflection_summary":"≤50字","final_conclusion":"修正后结论或原结论","timeliness_warning":str|null}}"""
        ...
        try:
            response = await self.llm_client.chat_completion(
                reflection_prompt, system_prompt="法律审查质量核查。仅返回 JSON。", temperature=0.1, max_tokens=1000
            )
            ...
            if idx != -1:
                ...
                self.logger.info(f"自我反思: passed={passed}, corrections={corrections}, summary={summary}")
    ...
    async def _llm_call(self, messages: list[dict]) -> dict:
        try:
            ...
            response = await self.llm_client.chat_completion_with_tools(
                messages=messages, tools=tool_schemas, temperature=0.1, max_tokens=2000
            )
            ...
            if role == "system":
                ...
                if tc_id in latest_tool_ids:
                    ...
                    if len(content) > max_tool_result_chars:
                        compressed.append({**msg, "content": content[:max_tool_result_chars] + "...(已截断)"})
                    else:
                        compressed.append(msg)
                ...
                if not is_last_two and len(content) > max_user_content_chars:
                    compressed.append({**msg, "content": content[:max_user_content_chars] + "...(已截断)"})
                else:
                    compressed.append(msg)
